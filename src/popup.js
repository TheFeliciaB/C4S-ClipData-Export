...
let keywords = "";
const unwantedText = "Select Related Categories";

if (Array.isArray(data.clipKeywords)) {
  keywords = data.clipKeywords
    .filter(item => item.trim() !== unwantedText)
    .join(", ");
} else if (typeof data.clipKeywords === "object" && data.clipKeywords !== null) {
  keywords = Object.values(data.clipKeywords)
    .filter(item => item.trim() !== unwantedText)
    .join(", ");
} else {
  keywords = data.clipKeywords.trim() !== unwantedText ? data.clipKeywords : "";
}
    
// Convert clipKeytypes to a comma-separated string, filtering out empty values.
let keytypes = "";
if (Array.isArray(data.clipKeytypes)) {
  keytypes = data.clipKeytypes
    .filter(item => item.trim() !== "")  // Filter out empty strings (after removing extra spaces)
    .join(", ");
} else if (typeof data.clipKeytypes === "object" && data.clipKeytypes !== null) {
  keytypes = Object.values(data.clipKeytypes)
